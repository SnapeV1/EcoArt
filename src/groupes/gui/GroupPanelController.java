...
Utilisateur current;
     public void setUtilisateur(Utilisateur current){
        this.current=current;
         ...
         System.out.println("GroupPanel" + current);
    }
    @FXML
    ...
    private void DeleteMem(MouseEvent event) throws SQLException {
         int IDOwner=(int)OwnerID;
        GF.DeleteMember(IDOwner,GPName,MemGTable,NomCol,PrenomCol,EmailCol,DateNColl,NumberCol) ;
...
return id;
    ...
    if((!"".equals(textFieldValue))&&((YearCB.getValue() == null))){
    String sql = "SELECT * FROM Utilisateur JOIN membre ON Utilisateur.id = membre.UserID WHERE Utilisateur.nom = ? AND membre.GroupID = ? AND membre.Role != 'Admin'";
   ...
   preparedStatement.setInt(2, Gid);
    ...
    String sql = "SELECT * FROM Utilisateur JOIN membre ON Utilisateur.id = membre.UserID WHERE YEAR(date_naissance) = ? AND membre.GroupID = ? AND membre.Role != 'Admin'";
...
private boolean isFrench = false;
    ...
    private void ConfirmGNAME(MouseEvent event) {
        ...
        if(NewName.isEmpty()){GroupNError.setText("invalid name");}
             ...
             try {
         String UpdateGNQuery="UPDATE groups SET nom = ? WHERE id = ?;";
    java.sql.PreparedStatement preparedStatement = MyConnection.getInstance().getCnx()
