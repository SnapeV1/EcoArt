...
int idOwn;
     public void setUtilisateur(Utilisateur current){
        this.current=current;
         ...
         int idOwn = (int) idOwner;
         System.out.println("ChatController" + current);
           try {
        ChatF.loadConversation(idOwn,conversationListView,conversationListView1);
    } catch (Exception ex) {
...
public void initialize(URL url, ResourceBundle rb) {
    ...
    @FXML
    private void SendMessage(MouseEvent event) throws SQLException, IOException, Exception  {
  ...
  if (!messageTextArea.getText().isEmpty()||(messageTextArea.getText().length()>1)) {
      ...
      MiscFunctions MF=new MiscFunctions();
          ...
          ChatF.Send(idOwner, EmailTXT, MF.Encrypt(messageTextArea.getText(),5));
        conversationListView.getItems().add("[NOW] "+messageTextArea.getText());
