...
if (result.isPresent() && result.get() == ButtonType.OK) {
   String sql = "SELECT GroupID FROM membre WHERE userId = ? AND Role = ?";
    ...
    String email = EmailCol.getCellData(index);
      ...
      String sqlDelete = "DELETE FROM membre WHERE GroupID = ? AND UserID = (SELECT id FROM User WHERE email = ?)";
...
if (result.isPresent() && result.get() == ButtonType.OK) {
    ...
    String sql = "SELECT email FROM User u JOIN membre m ON u.id = m.UserID WHERE m.GroupID = ? AND m.Role != 'Owner'";
...
try {
    ...
    return emails;
     ...
     public ObservableList<User> GetAllUserList() throws SQLException{
  ...
  String ownerName=null;
        ...
        String sql = "SELECT User.nom FROM User " +
                     "INNER JOIN membre ON User.id = membre.userId " +
